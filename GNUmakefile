...
GC:
		$(MAKE) XOPT="-DPTW32_BUILD_INLINED" CLEANUP="-D__CLEANUP_C -DPTW32_CLEANUP_C" XC_FLAGS="$(GC_CFLAGS)" OBJ="$(DLL_OBJS)" $(GC_DLL)
...
GC-static:
		$(MAKE) XOPT="-DPTW32_BUILD_INLINED -DPTW32_STATIC_LIB" CLEANUP="-D__CLEANUP_C -DPTW32_CLEANUP_C" XC_FLAGS="$(GC_CFLAGS)" OBJ="$(DLL_OBJS)" $(GC_INLINED_STATIC_STAMP)
